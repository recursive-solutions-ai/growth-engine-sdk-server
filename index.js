...
async function trySubscribeFromForm(db, settings, fields, data) {
  try {
    ...
    if (!isNewsletterForm && !optInEnabled) return;
    if (!isNewsletterForm) {
    ...
    const nameField = fields.find(
      (f) => ["name", "firstname", "first_name", "fullname", "full_name"].includes(
    ...
    const existing = await db.select().from(newsletterSubscribers).where(eq3(newsletterSubscribers.email, email)).limit(1);
    ...
    if (row.status === "unsubscribed") {
      await db.update(newsletterSubscribers).set({
        ...
        ...name ? { name } : {},
        updatedAt: now
      }).where(eq3(newsletterSubscribers.id, row.id));
      console.log("[GrowthEngine] newsletter opt-in \u2014 resubscribed via form |", email);
      return;
    }
    if (name && name !== row.name) {
      await db.update(newsletterSubscribers).set({ name, updatedAt: now }).where(eq3(newsletterSubscribers.id, row.id));
    }
  } catch (err) {
...
async function handleNewsletter(request, tursoUrl, tursoAuthToken) {
  try {
    ...
    if (action === "unsubscribe") {
      const rows = await db.select().from(newsletterSubscribers).where(eq5(newsletterSubscribers.email, email)).limit(1);
      const row2 = rows[0];
      if (row2 && row2.status !== "unsubscribed") {
        await db.update(newsletterSubscribers).set({ status: "unsubscribed", unsubscribedAt: now, updatedAt: now }).where(eq5(newsletterSubscribers.id, row2.id));
      }
...
async function getLandingPages(db, opts) {
  ...
  const rows = await db.select().from(landingPages).where(eq9(landingPages.status, "published")).orderBy(desc4(landingPages.createdAt)).limit(limit).offset(opts?.offset ?? 0);
  return rows.map(toPage2);
}
async function getLandingPage(db, slug) {
  const rows = await db.select().from(landingPages).where(eq9(landingPages.slug, slug)).limit(1);
  const row = rows[0];
...
async function getActiveForms(db) {
  const rows = await db.select().from(forms).where(eq10(forms.status, "active"));
  return rows.map(parseFormRow);
}
async function getFormBySlug(db, slug) {
  const rows = await db.select().from(forms).where(eq10(forms.slug, slug)).limit(1);
  const row = rows[0];
  if (!row) return null;
