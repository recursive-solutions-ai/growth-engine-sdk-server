...
import { eq as eq6, desc as desc3, and as and3, or as or2, count as drizzleCount3 } from "drizzle-orm";
var DEFAULT_LIMIT3 = 20;
var MAX_LIMIT3 = 5e3;
function resolveLimit2(limit) {
  if (limit === void 0) return DEFAULT_LIMIT3;
  if (limit <= 0) return MAX_LIMIT3;
...
async function getBlogPosts(db, opts) {
  ...
  const offset = opts?.offset ?? 0;
  const rows = await db.select().from(blogPosts).where(and3(
    eq6(blogPosts.status, "published"),
    eq6(blogPosts.language, locale)
  )).orderBy(desc3(blogPosts.createdAt)).limit(limit).offset(offset);
  return rows;
}
async function getBlogPost(db, slug, locale) {
  const slugMatch = or2(eq6(blogPosts.slug, slug), eq6(blogPosts.urlPath, slug));
  const conditions = [slugMatch];
  if (locale) conditions.push(eq6(blogPosts.language, locale));
  const rows = await db.select().from(blogPosts).where(and3(...conditions)).limit(1);
  return rows[0] ?? null;
}
async function getBlogPostCount(db, locale) {
  const conditions = [eq6(blogPosts.status, "published")];
  if (locale) conditions.push(eq6(blogPosts.language, locale));
  const result = await db.select({ count: drizzleCount3() }).from(blogPosts).where(and3(...conditions));
  return result[0]?.count ?? 0;
}
...
async function getBlogAuthor(db, slug) {
  const rows = await db.select().from(blogAuthors).where(eq6(blogAuthors.slug, slug)).limit(1);
  return rows[0] ?? null;
}
async function getBlogAuthorById(db, id) {
  const rows = await db.select().from(blogAuthors).where(eq6(blogAuthors.id, id)).limit(1);
  return rows[0] ?? null;
}
async function getAuthorPosts(db, authorSlug, opts) {
  const author = await db.select().from(blogAuthors).where(eq6(blogAuthors.slug, authorSlug)).limit(1);
  if (!author[0]) return [];
  const locale = opts?.locale ?? "en";
  const limit = resolveLimit2(opts?.limit);
  const offset = opts?.offset ?? 0;
  const rows = await db.select().from(blogPosts).where(and3(
    eq6(blogPosts.status, "published"),
    eq6(blogPosts.language, locale),
    eq6(blogPosts.authorId, author[0].id)
  )).orderBy(desc3(blogPosts.createdAt)).limit(limit).offset(offset);
  return rows;
}
...
async function getActiveForms(db) {
  const rows = await db.select().from(forms).where(eq7(forms.status, "active"));
  return rows.map(parseFormRow);
}
async function getFormBySlug(db, slug) {
  const rows = await db.select().from(forms).where(eq7(forms.slug, slug)).limit(1);
  const row = rows[0];
  if (!row) return null;
