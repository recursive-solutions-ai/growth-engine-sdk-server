...
import { eq as eq7, sql } from "drizzle-orm";
var CACHE_HEADERS3 = {
  "Cache-Control": "s-maxage=60, stale-while-revalidate=300"
};
...
async function handleUpdates(request, tursoUrl, tursoAuthToken) {
  try {
    ...
    if (action === "items") {
      ...
      }).from(updateItems).where(eq7(updateItems.listId, listRow.id)).orderBy(
...
import { eq as eq9, desc as desc4, and as and4, or as or2, count as drizzleCount3 } from "drizzle-orm";
