...
async function tryNotifyEmails(brainApiUrl, brainApiKey, formName, formSlug, fields, data, notifyEmails, senderEmail, customMessage) {
...
async function trySendConfirmationEmail(brainApiUrl, brainApiKey, senderEmail, formName, subject, message, data, replyTo) {
  ...
  try {
    ...
    if (request.method === "POST" && action === "submit" && slug) {
      ...
      if (notifyEmails && notifyEmails.length > 0) {
        ...
        void tryNotifyEmails(brainApiUrl, brainApiKey, form.name, slug, fields, data, notifyEmails, senderEmailForReply || void 0, settings?.notificationEmailMessage);
      }
      ...
      void tryTriggerPipelines(brainApiUrl, brainApiKey, form.id, data, db);
