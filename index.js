...
async function handleContent(request, tursoUrl, tursoAuthToken) {
  try {
    ...
    if (type === "landing_page") {
      if (slug) {
        const page = await db.select().from(landingPages).where(and(eq(landingPages.slug, slug), eq(landingPages.status, "published"))).limit(1);
        return Response.json(page[0] ? toPage(page[0]) : null, { headers: CACHE_HEADERS });
      }
      const limit = parseLimit(url.searchParams.get("limit"));
      const offset = parseOffset(url.searchParams.get("offset"));
      const pages = await db.select().from(landingPages).where(eq(landingPages.status, "published")).orderBy(desc(landingPages.createdAt)).limit(limit).offset(offset);
