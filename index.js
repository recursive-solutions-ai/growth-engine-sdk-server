...
var experts = sqliteTable("experts", {
  ...
  embedAllowedDomains: text("embed_allowed_domains", { mode: "json" }).$type().notNull().default([]),
  embedTheme: text("embed_theme", { mode: "json" }).$type().notNull().default({}),
  embedDisclaimer: text("embed_disclaimer"),
  embedRequireEmail: integer("embed_require_email", { mode: "boolean" }).notNull().default(false),
  createdAt: integer("created_at", { mode: "timestamp" }).notNull(),
  updatedAt: integer("updated_at", { mode: "timestamp" }).notNull()
});
