...
import { eq as eq4, desc as desc4, and as and4, or as or2, count as drizzleCount3 } from "drizzle-orm";
...
var blogPosts = sqliteTable("blog_posts", {
  ...
  // Fingerprint of the SOURCE post's translatable fields at the moment this
  // translation was produced. Only ever set on a translation (a row with
  // parent_post_id). When the original is edited its fingerprint moves on, and
  // the mismatch is what makes "this French page is out of date" a fact rather
  // than a guess. NULL = not a translation, or translated before tracking
  // existed (treated as "unknown", never as stale).
  sourceContentHash: text("source_content_hash"),
  // The source fields themselves (JSON: title, content, seoTitle, seoDesc,
  // keywords) at that same moment, written in the same statement as the hash.
  // What lets a drifted translation be RECONCILED (only the changed passages
  // patched in) instead of re-translated wholesale over somebody's edits. NULL
  // on translations made before it existed; see blog-translation-reconcile.ts.
...
var socialIdeas = sqliteTable("social_ideas", {
  ...
  platform: text("platform", { enum: ["linkedin", "x", "instagram", "facebook", "google"] }).notNull(),
  content: text("content").notNull(),
  ...
  // JSON: GoogleIdeaMeta — platform 'google' only (button + publish receipt).
  // NULL for every other platform. Column added in migration 0053.
...
var contactClicks = sqliteTable(
  "contact_clicks",
  {
    id: text("id").primaryKey(),
    kind: text("kind", { enum: ["phone", "email", "booking"] }).notNull(),
  ...
  (t) => ({
    createdIdx: index("idx_contact_clicks_created").on(t.createdAt),
    pagePathIdx: index("idx_contact_clicks_page_path").on(t.pagePath)
  })
);
...
var crmContacts = sqliteTable("crm_contacts", {
  ...
  source: text("source", { enum: ["form", "manual", "import", "sdk", "lead_finder", "embed", "api", "email"] }).notNull().default("manual"),
...
var crmDealSuggestions = sqliteTable(
  "crm_deal_suggestions",
  {
    id: text("id").primaryKey(),
    contactId: text("contact_id").notNull(),
    // Widening this list is type-only: the table carries no CHECK constraint,
    // so a new source needs no migration — only a label in the two
    // `Record<CrmDealSuggestionSource, string>` maps, which fail to compile
    // without it.
    sourceType: text("source_type", {
      enum: ["form", "lead_finder", "checklist_promote", "nurture_stop"]
    }).notNull(),
    sourceId: text("source_id"),
...
var nurtureFlows = sqliteTable("nurture_flows", {
  ...
  // How many people from the backlog get their FIRST email per UTC day (0059).
  // NULL → DEFAULT_DAILY_NEW_CONTACT_LIMIT. Follow-up steps never count.
  dailyNewContactLimit: integer("daily_new_contact_limit"),
  createdAt: integer("created_at", { mode: "timestamp" }).notNull(),
  updatedAt: integer("updated_at", { mode: "timestamp" }).notNull()
});
...
var nurtureRuns = sqliteTable(
  "nurture_runs",
  {
    ...
    // 'queued' = in the flow's backlog: created, nothing sent, no current node.
    // The backlog tick releases queued runs FIFO at the flow's daily pace. Type
    // level only — the column has no CHECK constraint, so no migration (0059).
    status: text("status", { enum: ["queued", "active", "stopped", "completed"] }).notNull().default("active"),
    stopReason: text("stop_reason"),
    // When a queued run left the backlog (0059). NULL for runs that never queued.
    releasedAt: integer("released_at", { mode: "timestamp" }),
    createdAt: integer("created_at", { mode: "timestamp" }).notNull(),
    updatedAt: integer("updated_at", { mode: "timestamp" }).notNull()
  },
  (t) => ({
    flowStatusCreatedIdx: index("idx_nurture_runs_flow_status_created").on(
      t.flowId,
      t.status,
      t.createdAt
    )
...
var updateLists = sqliteTable(
  "update_lists",
  {
    ...
    description: text("description"),
    // Per-language overrides for `name` / `description`, as a JSON object keyed
    // by language code: {"fr":{"name":"…","description":"…"}}. The columns above
    // stay the tenant's default language and are the fallback for any language
...
var updateItems = sqliteTable("update_items", {
  ...
  // Per-language overrides for `title` / `body`, as a JSON object keyed by
  // language code: {"fr":{"title":"…","body":"…"}}. The columns above stay the
  // tenant's default language and are the fallback for any language nobody has
  // written yet.
  translations: text("translations"),
  /**
   * @deprecated Vestigial. Updates are short-lived notices — retiring one means
   * deleting it — so draft/published was retired in migration 0055, which set
   * every row to 'published'. Nothing reads this; the column stays because
   * dropping it would be destructive, and the DB default stays 'draft' so the
   * TypeScript schema and the SQLite table do not drift. Every write path
   * passes 'published' explicitly. Do not reintroduce a filter on it.
   */
  status: text("status", { enum: ["draft", "published"] }).notNull().default("draft"),
  // When the item went live. Kept as the item's own date for ordering and for
  // the "Posted" column; no longer gates visibility.
  publishedAt: integer("published_at", { mode: "timestamp" }),
...
var BOOKING_CTA_DEFAULT_LABEL = "Book a call";
...
var BOOKING_CTA_SETTINGS_CONTACT_KEY = "bookingCta";
var BOOKING_CTA_LABEL_MAX = 80;
var BOOKING_CTA_LEAD_MAX = 200;
function cleanLine(value, max) {
  if (typeof value !== "string") return void 0;
  const cleaned = value.replace(/\s+/g, " ").trim().slice(0, max).trim();
  return cleaned || void 0;
}
function normalizeBookingBoxSettings(input) {
  if (!input || typeof input !== "object") return null;
  const raw = input;
  const out = {};
  if (typeof raw.enabled === "boolean") out.enabled = raw.enabled;
  const label = cleanLine(raw.label, BOOKING_CTA_LABEL_MAX);
  if (label) out.label = label;
  const lead = cleanLine(raw.lead, BOOKING_CTA_LEAD_MAX);
  if (lead) out.lead = lead;
  return Object.keys(out).length > 0 ? out : null;
}
function bookingBoxFromContact(contact) {
  if (!contact || typeof contact !== "object") return null;
  return normalizeBookingBoxSettings(
    contact[BOOKING_CTA_SETTINGS_CONTACT_KEY]
  );
}
function isBookingBoxDisabled(settings) {
  return settings?.enabled === false;
}
function withBookingBoxWording(cta, settings) {
  const out = { ...cta };
  if (settings?.label) out.buttonLabel = settings.label;
  if (settings?.lead) out.lead = settings.lead;
  return out;
}
...
// src/queries/keywords.ts
function parseStoredKeywords(raw) {
...
function withParsedKeywords(row) {
  return { ...row, keywords: parseStoredKeywords(row.keywords) };
}

// src/queries/content.ts
function toBlogPost(row) {
  return withParsedKeywords(row);
}
...
async function getBlogPost(db, slug, locale) {
  ...
  const conditions = [slugMatch, eq2(blogPosts.status, "published")];
...
async function getBlogAuthor(db, slug) {
  const rows = await db.select().from(blogAuthors).where(eq2(blogAuthors.slug, slug)).limit(1);
  return rows[0] ?? null;
}
async function getBlogAuthorById(db, id) {
  const rows = await db.select().from(blogAuthors).where(eq2(blogAuthors.id, id)).limit(1);
  return rows[0] ?? null;
}
...
async function publishedPosts(db, locale) {
  const rows = await db.select().from(blogPosts).where(and3(eq3(blogPosts.status, "published"), eq3(blogPosts.language, locale))).orderBy(desc3(blogPosts.createdAt)).limit(MAX_POSTS);
  return rows.map(toBlogPost);
}
...
async function getBookingCallToAction(db) {
  ...
  const box = bookingBoxFromContact(business?.contact);
  if (isBookingBoxDisabled(box)) return null;
  const candidates = [];
  try {
    const rows = await db.select({ slug: forms.slug, name: forms.name }).from(forms).where(eq3(forms.status, "active")).orderBy(desc3(forms.updatedAt)).limit(10);
  ...
  const cta = resolveBookingCallToAction(candidates, bookingUrlFromContact(business?.contact));
  return cta ? withBookingBoxWording(cta, box) : null;
}
...
function parseLimit(limitParam) {
  ...
  if (isNaN(n) || n < 0) return DEFAULT_LIMIT3;
  if (n === 0) return MAX_LIMIT3;
  ...
  try {
    ...
    if (type === "blog") {
      ...
      if (slug) {
        ...
        const conditions = [slugMatch, eq4(blogPosts.status, "published")];
        if (locale) conditions.push(eq4(blogPosts.language, locale));
        const post = await db.select().from(blogPosts).where(and4(...conditions)).limit(1);
        return Response.json(post[0] ? toBlogPost(post[0]) : null, { headers: CACHE_HEADERS });
      }
      ...
      if (authorSlug) {
        ...
        const where2 = and4(
          ...
          eq4(blogPosts.language, language2),
          eq4(blogPosts.authorId, author[0].id)
        );
        const posts2 = await db.select().from(blogPosts).where(where2).orderBy(desc4(blogPosts.createdAt)).limit(limit2).offset(offset2);
        return Response.json(posts2.map(toBlogPost), { headers: CACHE_HEADERS });
      }
      ...
      const where = and4(
        eq4(blogPosts.status, "published"),
        eq4(blogPosts.language, language)
      );
      const posts = fields ? await db.select(fields).from(blogPosts).where(where).orderBy(desc4(blogPosts.createdAt)).limit(limit).offset(offset) : (await db.select().from(blogPosts).where(where).orderBy(desc4(blogPosts.createdAt)).limit(limit).offset(offset)).map(toBlogPost);
    ...
    if (type === "landing_page") {
      if (slug) {
        const page = await db.select().from(landingPages).where(and4(eq4(landingPages.slug, slug), eq4(landingPages.status, "published"))).limit(1);
        return Response.json(page[0] ? toPage(page[0]) : null, { headers: CACHE_HEADERS });
      }
      const limit = parseLimit(url.searchParams.get("limit"));
      const offset = parseOffset(url.searchParams.get("offset"));
      const pages = await db.select().from(landingPages).where(eq4(landingPages.status, "published")).orderBy(desc4(landingPages.createdAt)).limit(limit).offset(offset);
      return Response.json(pages.map(toPage), { headers: CACHE_HEADERS });
    }
    if (type === "blog-authors") {
      if (slug) {
        const author = await db.select().from(blogAuthors).where(eq4(blogAuthors.slug, slug)).limit(1);
        return Response.json(author[0] ?? null, { headers: CACHE_HEADERS });
      }
      const authors = await db.select().from(blogAuthors).orderBy(desc4(blogAuthors.createdAt));
      return Response.json(authors, { headers: CACHE_HEADERS });
    }
    if (type === "social") {
      if (platform === "instagram" || platform === "linkedin" || platform === "facebook") {
        const posts2 = await db.select().from(socialPosts).where(eq4(socialPosts.platform, platform)).orderBy(desc4(socialPosts.createdAt)).limit(20);
        return Response.json(posts2, { headers: CACHE_HEADERS });
      }
      const posts = await db.select().from(socialPosts).orderBy(desc4(socialPosts.createdAt)).limit(20);
      return Response.json(posts, { headers: CACHE_HEADERS });
    }
    if (type === "newsletter") {
      const countOnly = url.searchParams.get("count") === "true";
      if (countOnly) {
        const result = await db.select({ count: drizzleCount3() }).from(newsletterIssues).where(eq4(newsletterIssues.status, "sent"));
        return Response.json({ count: result[0]?.count ?? 0 }, { headers: CACHE_HEADERS });
      }
      if (slug) {
        const rows2 = await db.select().from(newsletterIssues).where(and4(eq4(newsletterIssues.slug, slug), eq4(newsletterIssues.status, "sent"))).limit(1);
        return Response.json(
      ...
      const limit = parseLimit(url.searchParams.get("limit"));
      const offset = parseOffset(url.searchParams.get("offset"));
      const rows = await db.select().from(newsletterIssues).where(eq4(newsletterIssues.status, "sent")).orderBy(desc4(newsletterIssues.sentAt)).limit(limit).offset(offset);
...
// ../types/src/update-list.ts
function resolveTranslated(base, override) {
  if (override === void 0 || override.trim() === "") return base;
  return override;
}
function parseTranslations(raw) {
  ...
  try {
    ...
    console.error("[updates] ignoring malformed translations JSON:", err);
    return null;
  }
}
...
// ../types/src/mailbox-connection.ts
var MAILBOX_PROVIDER_PRESETS = [
  {
    id: "gmail",
    ...
    passwordHelp: 'Google will not accept your normal password here. Turn on 2-step verification on the account, then create an "app password" and paste that instead. It is a 16-character code that only this connection uses, and you can revoke it at any time without changing your real password.',
    helpUrl: "https://support.google.com/accounts/answer/185833"
  },
  {
    id: "office365",
    label: "Outlook.com or Microsoft 365",
    imap: { host: "outlook.office365.com", port: 993, security: "ssl" },
    smtp: { host: "smtp.office365.com", port: 587, security: "starttls" },
    passwordHelp: 'Be aware that many Microsoft 365 accounts cannot be connected this way at all. Microsoft switched off password-based IMAP and SMTP for most business accounts, and only your own IT administrator can turn it back on for a mailbox. If the connection fails with "your provider has switched this off", that is what happened. On a personal Outlook.com account, create an app password and use that instead of your normal password.',
    helpUrl: "https://learn.microsoft.com/en-us/exchange/clients-and-mobile-in-exchange-online/authenticated-client-smtp-submission"
  },
  {
    id: "yahoo",
    label: "Yahoo Mail",
    ...
    passwordHelp: 'Yahoo will not accept your normal password here. Generate an "app password" in your Yahoo account security settings and paste that instead.',
  ...
  },
  {
    ...
    passwordHelp: 'Apple will not accept your normal password here. Create an "app-specific password" on your Apple Account page and paste that instead. Your username is usually the full iCloud address.',
  ...
  },
  {
    ...
    passwordHelp: "If the account has two-factor authentication turned on, Zoho needs an application password rather than your normal one. You can create one in Zoho account security. Check too that IMAP is enabled under Mail Settings, because Zoho ships it off on some plans.",
  ...
  },
  {
    ...
    passwordHelp: "Fastmail requires an app password for mail apps. Create one in Settings, Privacy and Security, with access to Mail, and paste it here.",
  ...
  },
  {
    ...
    passwordHelp: 'Your email provider or whoever set up your email can give you these settings. Look for a page called "IMAP and SMTP settings" or "mail app settings". Most providers use port 993 for reading and 465 or 587 for sending.',
    helpUrl: null
  }
];
var MAILBOX_PRESET_MAP = new Map(
  MAILBOX_PROVIDER_PRESETS.map((p) => [p.id, p])
);
...
async function tryCreateCrmContact(db, submissionId, data) {
  try {
    ...
    if (email) {
      const existing = await db.select({ id: crmContacts.id }).from(crmContacts).where(eq5(crmContacts.email, String(email))).limit(1);
...
async function tryTriggerPipelines(brainApiUrl, brainApiKey, formId, submissionData, db) {
  try {
    ...
    if (!email) return;
    const rows = await db.select({ id: crmContacts.id, firstName: crmContacts.firstName, lastName: crmContacts.lastName }).from(crmContacts).where(eq5(crmContacts.email, String(email))).limit(1);
...
async function handleForms(request, tursoUrl, tursoAuthToken, brainApiUrl, brainApiKey) {
  try {
    ...
    if (request.method === "POST" && action === "submit" && slug) {
      ...
      const formRows = await db.select().from(forms).where(eq5(forms.slug, slug)).limit(1);
...
var PUBLIC_STATUS_JOB_TYPES = /* @__PURE__ */ new Set(["blog.generate", "social.sync"]);
...
async function handleJobs(request, brainApiKey) {
  if (request.method !== "GET") {
    console.warn(`[GrowthEngine] jobs route: refused ${request.method} (enqueue jobs server-side with enqueueJob)`);
    return Response.json(
      {
        error: "Method not allowed. Jobs cannot be created through the public /api/rs/jobs route; call enqueueJob() from @growth-engine/sdk-server in server code instead."
      },
  ...
  try {
    ...
    if (!res.ok) {
      if (res.status !== 404) {
        console.error("[GrowthEngine] jobs route: Brain returned", res.status, "for job", jobId);
      }
      return Response.json(
        { error: res.status === 404 ? "Job not found" : "Job status unavailable" },
        { status: res.status === 404 ? 404 : 502 }
      );
    }
    const data = await res.json();
    if (typeof data.type !== "string" || !PUBLIC_STATUS_JOB_TYPES.has(data.type)) {
...
// src/handler/routes/contact-clicks.ts
import { and as and5, eq as eq6, gte } from "drizzle-orm";
var KINDS = ["phone", "email", "booking"];
var MAX_LEN = 2048;
...
async function handleContactClick(request, tursoUrl, tursoAuthToken) {
  if (request.method !== "POST") {
    return Response.json({ error: "Method not allowed" }, { status: 405 });
  }
  try {
    ...
    const href = truncate(body.href);
    if (!href) {
      return Response.json({ error: "href is required" }, { status: 400 });
    }
    const pagePath = truncate(body.pagePath);
    if (!pagePath) {
      return Response.json({ error: "pagePath is required" }, { status: 400 });
    }
    const sessionId = truncate(body.sessionId);
    const db = getClientDb(tursoUrl, tursoAuthToken);
    if (sessionId) {
      const since = new Date(Date.now() - DEDUPE_WINDOW_MS);
      const recent = await db.select({ id: contactClicks.id }).from(contactClicks).where(
        and5(
          eq6(contactClicks.sessionId, sessionId),
          eq6(contactClicks.href, href),
          gte(contactClicks.createdAt, since)
        )
      ).limit(1);
      if (recent.length > 0) {
        return Response.json({ success: true, deduped: true }, { status: 200 });
      }
    }
    const id = crypto.randomUUID();
    ...
    console.error("[GrowthEngine] contact-click route error:", err);
    return Response.json({ error: "Internal server error" }, { status: 500 });
  }
}
...
async function handleCrm(request, tursoUrl, tursoAuthToken) {
  try {
    ...
    if (request.method === "POST" && action === "push-lead") {
      ...
      if (body.email) {
        const existing = await db.select({ id: crmContacts.id }).from(crmContacts).where(eq7(crmContacts.email, body.email.trim())).limit(1);
...
async function handleNewsletter(request, tursoUrl, tursoAuthToken) {
  try {
    ...
    if (action === "unsubscribe") {
      const rows = await db.select().from(newsletterSubscribers).where(eq8(newsletterSubscribers.email, email)).limit(1);
      const row2 = rows[0];
      if (row2 && row2.status !== "unsubscribed") {
        await db.update(newsletterSubscribers).set({ status: "unsubscribed", unsubscribedAt: now, updatedAt: now }).where(eq8(newsletterSubscribers.id, row2.id));
      }
      return Response.json({ ok: true, unsubscribed: true });
    }
    const existing = await db.select().from(newsletterSubscribers).where(eq8(newsletterSubscribers.email, email)).limit(1);
    const row = existing[0];
    if (!row) {
      await db.insert(newsletterSubscribers).values({
        id: crypto.randomUUID(),
      ...
      await db.update(newsletterSubscribers).set({
        ...
        ...name ? { name } : {},
        updatedAt: now
      }).where(eq8(newsletterSubscribers.id, row.id));
    } else if (name && name !== row.name) {
      await db.update(newsletterSubscribers).set({ name, updatedAt: now }).where(eq8(newsletterSubscribers.id, row.id));
    }
...
import { eq as eq9, sql } from "drizzle-orm";
var CACHE_HEADERS3 = {
  "Cache-Control": "s-maxage=60, stale-while-revalidate=300"
};
...
function resolveList(row, lang) {
  const base = { id: row.id, name: row.name, slug: row.slug, description: row.description };
  if (!lang) return { ...base, language: null };
  const entry = parseTranslations(row.translations)?.[lang];
  if (!entry) return { ...base, language: null };
  const name = resolveTranslated(row.name, entry.name);
  const description = resolveTranslated(row.description ?? "", entry.description ?? void 0);
  const translated = name !== row.name || description !== (row.description ?? "");
  return {
    ...base,
    name,
...
function resolveItem(row, lang) {
  ...
  const entry = parseTranslations(row.translations)?.[lang];
  if (!entry) return { ...base, language: null };
  const title = resolveTranslated(row.title, entry.title);
  const body = resolveTranslated(row.body, entry.body);
  const translated = title !== row.title || body !== row.body;
  return { ...base, title, body, language: translated ? lang : null };
}
function parseLimit2(limitParam) {
  if (limitParam === null) return DEFAULT_LIMIT4;
  const n = Number.parseInt(limitParam, 10);
  if (Number.isNaN(n) || n < 1) return DEFAULT_LIMIT4;
  return Math.min(n, MAX_LIMIT4);
}
  ...
  try {
    ...
    if (action === "lists") {
      const rows = await db.select({
        ...
        translations: updateLists.translations
      }).from(updateLists).orderBy(updateLists.name);
      const lists = rows.map((row) => resolveList(row, lang));
    ...
    if (action === "items") {
      ...
      const listRows = await db.select({
        ...
        description: updateLists.description,
        translations: updateLists.translations
      }).from(updateLists).where(eq9(updateLists.slug, listSlug)).limit(1);
      const listRow = listRows[0];
      if (!listRow) {
        return Response.json({ error: "Update list not found" }, { status: 404 });
      }
      const list = resolveList(listRow, lang);
      ...
      const itemRows = await db.select({
        ...
        translations: updateItems.translations
      }).from(updateItems).where(eq9(updateItems.listId, listRow.id)).orderBy(
        sql`coalesce(${updateItems.eventDate}, ${updateItems.publishedAt}, ${updateItems.createdAt}) desc`
      ).limit(limit);
      const items = itemRows.map((row) => resolveItem(row, lang));
      return Response.json(
        { list, items },
        { headers: CACHE_HEADERS3 }
      );
...
import { asc, eq as eq10 } from "drizzle-orm";
var CACHE_HEADERS4 = {
  "Cache-Control": "s-maxage=60, stale-while-revalidate=300"
};
async function handleVariables(request, tursoUrl, tursoAuthToken) {
  try {
    ...
    const db = getClientDb(tursoUrl, tursoAuthToken);
    const rows = await db.select({ key: variables.key, value: variables.value }).from(variables).where(eq10(variables.scope, "company")).orderBy(asc(variables.updatedAt));
    const map = {};
    for (const row of rows) {
...
var CONTACT_CLICKS_SUPPORTED = true;
  ...
  async function handleRequest(request) {
    ...
    if (route === "sdk-status") {
      return Response.json({
        ...
        structuralLinks,
        contactClicks: CONTACT_CLICKS_SUPPORTED,
        disabled: true
      });
    }
    return Response.json({ error: "Not found" }, { status: 404 });
...
function GrowthEngineHandler(options) {
  ...
  async function handleRequest(request) {
    ...
    if (route === "contact-click") {
      return handleContactClick(request, dbUrl, dbToken);
    }
    if (route.startsWith("forms")) {
...
// src/jobs.ts
var EnqueueJobError = class extends Error {
  status;
...
async function enqueueJob(input, options = {}) {
  if ("window" in globalThis) {
    throw new EnqueueJobError(
      "enqueueJob is server-only: it uses BRAIN_API_KEY and must never run in the browser",
      null
    );
  }
  const env = globalThis.process?.env;
  const brainApiUrl = (options.brainApiUrl ?? env?.BRAIN_API_URL)?.trim();
  const brainApiKey = (options.brainApiKey ?? env?.BRAIN_API_KEY)?.trim();
  if (!brainApiUrl || !brainApiKey) {
    throw new EnqueueJobError("enqueueJob requires BRAIN_API_URL and BRAIN_API_KEY", null);
  }
  let res;
  try {
    res = await fetch(`${brainApiUrl.replace(/\/+$/, "")}/api/jobs`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${brainApiKey}`
    ...
    console.error("[GrowthEngine] enqueueJob: request to Brain failed", { type: input.type }, err);
    throw new EnqueueJobError("Could not reach Brain to enqueue the job", null);
  }
  const data = await res.json().catch(() => null);
  if (!res.ok || !data?.jobId) {
    const message = data?.error ?? `Brain returned HTTP ${res.status}`;
    console.error("[GrowthEngine] enqueueJob failed", { type: input.type, status: res.status, message });
    throw new EnqueueJobError(message, res.status);
  }
  return { jobId: data.jobId, traceId: data.traceId ?? "", status: "queued" };
}
function triggerBlogGen(payload, options) {
  return enqueueJob({ type: "blog.generate", payload: { ...payload } }, options);
}
function triggerSocialSync(platforms, options) {
  return enqueueJob(
    { type: "social.sync", payload: platforms ? { platforms } : {} },
    options
  );
}
...
async function getLandingPages(db, opts) {
  ...
  const rows = await db.select().from(landingPages).where(eq11(landingPages.status, "published")).orderBy(desc5(landingPages.createdAt)).limit(limit).offset(opts?.offset ?? 0);
  return rows.map(toPage2);
}
async function getLandingPage(db, slug) {
  const rows = await db.select().from(landingPages).where(eq11(landingPages.slug, slug)).limit(1);
  const row = rows[0];
...
async function getActiveForms(db) {
  const rows = await db.select().from(forms).where(eq12(forms.status, "active"));
  return rows.map(parseFormRow);
}
async function getFormBySlug(db, slug) {
  const rows = await db.select().from(forms).where(eq12(forms.slug, slug)).limit(1);
  const row = rows[0];
  if (!row) return null;
