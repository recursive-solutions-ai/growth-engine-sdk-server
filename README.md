...
Read-only feeds of short markdown items (announcements, events, changelogs). POST returns 405.

| Param | Type | Description |
|-------|------|-------------|
| `action` | `'lists'` \| `'items'` | Operation to perform |
| `list` | string | List slug (required for `items`) |
| `limit` | number | Items to return (default: 20, max: 100) |

**GET `?action=lists`** -- Returns `{ lists: [{ id, name, slug, description }] }`, ordered by name.

**GET `?action=items&list=announcements`** -- Returns `{ list: { id, name, slug, description }, items: [{ id, title, body, imageUrl, eventDate, publishedAt }] }`.

Only `status = 'published'` items are returned; drafts and internal fields (`status`, `sortOrder`, timestamps) never leave the Brain. Items are ordered by `COALESCE(event_date, published_at, created_at) DESC`. An unknown list slug returns `404 { error }`.

### GET `/api/rs/variables`

Returns tenant-wide variables as a flat map: `{ variables: { [key]: value } }`.

**Company scope only.** Rows with `scope = 'user'` belong to a single portal user and are never served here — this endpoint is public to every site visitor. `note`, `id` and `userId` are dropped for the same reason. POST returns 405.

### GET `/api/rs/jobs/{jobId}`

Check job status (proxied to Brain API). Returns a summary only, `{ jobId, type, status, createdAt }`: this route is public to every site visitor, so a job's result, error and payload never leave the server, and only `blog.generate` / `social.sync` jobs are visible (anything else is `404`).

**POST returns 405.** Jobs cannot be created through the public route: it used to forward any anonymous body to Brain with the site's API key. Enqueue from server code instead:

```ts
import { enqueueJob, triggerBlogGen } from '@growth-engine/sdk-server'

// In a route handler / server action behind YOUR OWN auth check:
await triggerBlogGen({ topic: 'Summer menu' })
await enqueueJob({ type: 'social.sync', payload: { platforms: ['instagram'] } })
```

Both read `BRAIN_API_URL` and `BRAIN_API_KEY` from the environment (or take `{ brainApiUrl, brainApiKey }` as a second argument) and throw `EnqueueJobError` on failure.
...
| `enqueueJob({ type, payload?, priority? }, options?)` | Server-only. Enqueue a `blog.generate` / `social.sync` job with the API key |
| `triggerBlogGen(payload, options?)` / `triggerSocialSync(platforms?, options?)` | Server-only shorthands for `enqueueJob` |
