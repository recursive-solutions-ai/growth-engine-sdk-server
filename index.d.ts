...
import { JobTriggerResponse, BlogGeneratePayload, SocialSyncPayload, BlogPost, BlogAuthor, Page, Form, BusinessConfig, RobotsPolicyResponse } from '@growth-engine/types';
...
interface GrowthEngineHandlerOptions {
    ...
    /**
     * Does THIS repo's blog actually render the structural internal links —
     * `RelatedArticles` + `TopicChips` on the post page and a working
     * `/blog/topic/[slug]` route?
     *
     * The SDK cannot know: those live in the site's own templates, and bumping
     * `@growth-engine/sdk-{client,server}` ports none of them. So the repo
     * declares it, `/api/rs/sdk-status` reports it, and Brain's orphan check
     * gates on that instead of on the version number. Leave it unset (or false)
     * until the templates are wired — Brain fails closed and counts body links
     * only, which over-reports orphans rather than hiding a blog where every
     * post is a true orphan.
     */
    structuralLinks?: boolean;
}
...
/** Job types a client site may enqueue. Brain enforces its own allowlist too. */
type SdkJobType = 'blog.generate' | 'social.sync';
interface EnqueueJobInput {
    type: SdkJobType;
    payload?: Record<string, unknown> | undefined;
    priority?: 'urgent' | 'background' | undefined;
}
interface EnqueueJobOptions {
    /** Defaults to process.env.BRAIN_API_URL. */
    brainApiUrl?: string | undefined;
    /** Defaults to process.env.BRAIN_API_KEY. */
    brainApiKey?: string | undefined;
}
declare class EnqueueJobError extends Error {
    readonly status: number | null;
    constructor(message: string, status: number | null);
}
declare function enqueueJob(input: EnqueueJobInput, options?: EnqueueJobOptions): Promise<JobTriggerResponse>;
/** Server-side replacement for sdk-client's removed `triggerBlogGen`. */
declare function triggerBlogGen(payload: BlogGeneratePayload, options?: EnqueueJobOptions): Promise<JobTriggerResponse>;
/** Server-side replacement for sdk-client's removed `triggerSocialSync`. */
declare function triggerSocialSync(platforms?: SocialSyncPayload['platforms'], options?: EnqueueJobOptions): Promise<JobTriggerResponse>;
...
/**
 * Where the "Book a call" box under every post sends the reader. The owner's
 * own booking link (mirrored from the portal's business settings into this
 * tenant's `business_config.contact` JSON) wins outright; otherwise it is built
 * from the same real destinations the landing-page writer sees — active forms,
 * then the business phone, then its email — and chosen by the same rule. Null
 * when the business has published none of them, in which case the template
 * hides the slot.
 *
 * The owner controls the box itself (`contact.bookingCta`, mirrored from the
 * portal next to the booking link): switched off, this returns null so every
 * template hides it; with their own wording, the result carries `buttonLabel`
 * and `lead`, which the template prefers over its dictionary default.
 */
...
export { type AiCrawlerRule, type BlogTopicPage, type ClientDb, EnqueueJobError, type EnqueueJobInput, type EnqueueJobOptions, type GetAiCrawlerRulesOptions, type GetBlogPostsOptions, type GetLandingPagesOptions, type GetNewsletterIssuesOptions, type GetRelatedPostsOptions, GrowthEngineHandler, type GrowthEngineHandlerOptions, type PublicNewsletterIssue, type SdkJobType, buildAiCrawlerRules, enqueueJob, fetchRobotsPolicy, getActiveForms, getAiCrawlerRules, getAuthorPosts, getBlogAuthor, getBlogAuthorById, getBlogAuthors, getBlogPost, getBlogPostCount, getBlogPosts, getBlogTopic, getBlogTopics, getBlogTopicsForPost, getBookingCallToAction, getBusinessConfig, getClientDb, getFormBySlug, getLandingPage, getLandingPages, getNewsletterIssue, getNewsletterIssueCount, getNewsletterIssues, getRelatedPosts, triggerBlogGen, triggerSocialSync };
